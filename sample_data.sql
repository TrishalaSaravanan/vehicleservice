-- Sample data for testing admin stats
-- First add some users with mechanic role
INSERT INTO user (id, email, password_hash, role, is_active, created_at) VALUES
('mech_001', 'john.doe@mechniq.com', '$2a$10$example.hash.here', 'mechanic', TRUE, '2024-07-01 10:00:00'),
...
-- Then add mechanic profiles
INSERT INTO mechanic (id, user_id, employee_id, name, phone, address, experience, is_active, created_at) VALUES
('mech_001', 'mech_001', 'EMP001', 'John Doe', '+1-555-0101', '123 Main St, City', 5, TRUE, '2024-07-01 10:00:00'),
...
-- Add some sample customers for appointments
INSERT INTO user (id, email, password_hash, role, is_active, created_at) VALUES
('cust_001', 'customer1@example.com', '$2a$10$example.hash.here', 'customer', TRUE, '2024-07-01 09:00:00'),
('cust_002', 'customer2@example.com', '$2a$10$example.hash.here', 'customer', TRUE, '2024-07-02 09:00:00')
ON DUPLICATE KEY UPDATE email = VALUES(email);

INSERT INTO customer (id, user_id, name, phone, address, is_active, created_at) VALUES
('cust_001', 'cust_001', 'Customer One', '+1-555-0201', '111 Customer St, City', TRUE, '2024-07-01 09:00:00'),
('cust_002', 'cust_002', 'Customer Two', '+1-555-0202', '222 Customer Ave, City', TRUE, '2024-07-02 09:00:00')
ON DUPLICATE KEY UPDATE name = VALUES(name);
...
('veh_002', 'cust_002', 'Honda', 'Civic', 2019, '2HGFC2F56KH123456', 'XYZ789', 32000)
ON DUPLICATE KEY UPDATE make = VALUES(make);

-- Add some sample appointments (today and yesterday for trend calculation)
...
-- Check the data
SELECT 'Total mechanics:' as info, COUNT(*) as count FROM mechanic WHERE is_active = TRUE;
SELECT 'Today appointments:' as info, COUNT(*) as count FROM mechanic_appointment WHERE DATE(date) = CURDATE();
SELECT 'Today new bookings:' as info, COUNT(*) as count FROM mechanic_appointment WHERE DATE(created_at) = CURDATE();
SELECT 'Yesterday appointments:' as info, COUNT(*) as count FROM mechanic_appointment WHERE DATE(date) = DATE_SUB(CURDATE(), INTERVAL 1 DAY);

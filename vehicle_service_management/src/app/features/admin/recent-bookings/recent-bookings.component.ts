...
import { LocalStorageService, Appointment, Customer } from '../../../services/local-storage.service';
...
export class RecentBookingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    const appointments = this.localStorage.getAppointments();
    this.bookings = appointments.map((appt, idx) => {
      const customer = this.localStorage.getUserById(appt.customerId) as Customer;
      let status: 'new' | 'assigned' | 'completed' = 'new';
      if (appt.status === 'Confirmed') status = 'assigned';
      if (appt.status === 'Completed') status = 'completed';
      const bookingId = appt.id.startsWith('#BK') ? appt.id : `#BK${(100 + idx + 1).toString().padStart(2, '0')}`;
      return {
        id: bookingId,
        customer: {
          name: customer?.name || 'Unknown',
          phone: customer?.phone || '',
          avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(customer?.name || 'U')}`
        },
    ...
    console.log('Loaded bookings:', this.bookings); // Debug log
    this.filteredBookings = [...this.bookings]; // Always show all bookings
  }
